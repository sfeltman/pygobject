...
class _ConnectDataTestBase(object):
    ...
    def run_connect_data_test(self, emit_args, user_data, flags=0):
        obj = self.Object()
        callback_args = []
        ...
        obj.connect('sig-with-int64-prop', callback, flags=flags, *user_data)
        obj.emit('sig-with-int64-prop', *emit_args)
        self.assertEqual(len(callback_args), 1)
        return callback_args[0]

    def test_connect_data_0(self):
        obj, value = self.run_connect_data_test([GObject.G_MAXINT64], user_data=[])
        self.assertIsInstance(obj, self.Object)
        self.assertEqual(value, GObject.G_MAXINT64)

    def test_connect_data_1(self):
        obj, value, data = self.run_connect_data_test([GObject.G_MAXINT64],
                                                      user_data=['mydata'])
        self.assertIsInstance(obj, self.Object)
        self.assertEqual(value, GObject.G_MAXINT64)
        self.assertEqual(data, 'mydata')

    def test_connect_data_after_0(self):
        obj, value = self.run_connect_data_test([GObject.G_MAXINT64],
                                                user_data=[],
                                                flags=GObject.ConnectFlags.AFTER)
        self.assertIsInstance(obj, self.Object)
        self.assertEqual(value, GObject.G_MAXINT64)

    def test_connect_data_after_1(self):
        obj, value, data = self.run_connect_data_test([GObject.G_MAXINT64],
                                                      user_data=['mydata'],
                                                      flags=GObject.ConnectFlags.AFTER)
        self.assertIsInstance(obj, self.Object)
    ...
    def test_connect_data_swaped_0(self):
        # Swapped only works with a single user data argument.
        with self.assertRaises(ValueError):
            self.run_connect_data_test([GObject.G_MAXINT64],
                                       user_data=[],
                                       flags=GObject.ConnectFlags.SWAPPED)

    def test_connect_data_swaped_1(self):
        # Notice obj and data are reversed in the return.
        data, value, obj = self.run_connect_data_test([GObject.G_MAXINT64],
                                                      user_data=['mydata'],
                                                      flags=GObject.ConnectFlags.SWAPPED)
        self.assertIsInstance(obj, self.Object)
        self.assertEqual(value, GObject.G_MAXINT64)
    ...
    def test_connect_data_swaped_2(self):
        # Swapped only works with a single user data argument.
        with self.assertRaises(ValueError):
            self.run_connect_data_test([GObject.G_MAXINT64],
                                       user_data=[1, 2],
    ...
    def test_connect_data_after_and_swapped_0(self):
        # Swapped only works with a single user data argument.
        with self.assertRaises(ValueError):
            self.run_connect_data_test([GObject.G_MAXINT64],
                                       user_data=[],
    ...
    def test_connect_data_after_and_swapped_1(self):
        # Notice obj and data are reversed in the return.
        data, value, obj = self.run_connect_data_test([GObject.G_MAXINT64],
                                                      user_data=['mydata'],
                                                      flags=GObject.ConnectFlags.AFTER | GObject.ConnectFlags.SWAPPED)
    ...
    def test_connect_data_after_and_swapped_2(self):
        # Swapped only works with a single user data argument.
        with self.assertRaises(ValueError):
            self.run_connect_data_test([GObject.G_MAXINT64],
                                       user_data=[],
